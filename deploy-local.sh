...
#
# Deploys git-changed files as copies to .claude/ directory.
...
TARGET_CLAUDE_DIR="$SCRIPT_DIR/.claude"
...
TARGET_RULES_DIR="$TARGET_CLAUDE_DIR/rules"

echo -e "${BLUE}======================================${NC}"
echo -e "${BLUE}  Local Test Deployment${NC}"
echo -e "${BLUE}======================================${NC}"
echo ""
...
deploy_agents() {
  ...
  if [ -n "$changed" ]; then
    while IFS= read -r line; do
      ...
      if [ "$status" = "D" ]; then
        if remove_file "$TARGET_AGENTS_DIR/$(basename "$file")" "$(basename "$file")"; then
          ((deleted++)) || true || true
        fi
      else
        if copy_file "$SCRIPT_DIR/$file" "$TARGET_AGENTS_DIR/$(basename "$file")" "false"; then
          ((added++)) || true || true
        fi
      fi
    done <<< "$changed"
...
deploy_commands() {
  ...
  if [ -n "$changed" ]; then
    while IFS= read -r line; do
      ...
      if [ "$status" = "D" ]; then
        if remove_file "$TARGET_COMMANDS_DIR/$(basename "$file")" "$(basename "$file")"; then
          ((deleted++)) || true
        fi
      else
        if copy_file "$SCRIPT_DIR/$file" "$TARGET_COMMANDS_DIR/$(basename "$file")" "false"; then
          ((added++)) || true
        fi
      fi
    done <<< "$changed"
...
deploy_hooks() {
  ...
  if [ -n "$changed" ]; then
    while IFS= read -r line; do
      ...
      if [ "$status" = "D" ]; then
        if remove_file "$TARGET_HOOKS_DIR/$file_name" "$file_name"; then
          ((deleted++)) || true
        fi
      else
        if copy_file "$SCRIPT_DIR/$file" "$TARGET_HOOKS_DIR/$file_name" "true"; then
          ((added++)) || true
        fi
      fi
    done <<< "$changed"
  ...
  if [ -n "$scripts_changed" ]; then
    mkdir -p "$TARGET_CLAUDE_DIR/scripts/hooks"
    while IFS= read -r line; do
      local status=$(echo "$line" | cut -c1)
      local file=$(echo "$line" | cut -c2- | sed 's/^[[:space:]]*//')
      local file_name=$(basename "$file")
      ...
      if [ "$status" = "D" ]; then
        if remove_file "$TARGET_CLAUDE_DIR/scripts/hooks/$file_name" "scripts/hooks/$file_name"; then
          ((deleted++)) || true
        fi
      else
        if copy_file "$SCRIPT_DIR/$file" "$TARGET_CLAUDE_DIR/scripts/hooks/$file_name" "true"; then
          ((added++)) || true
        fi
      fi
...
deploy_rules() {
  ...
  if [ -n "$changed" ]; then
    while IFS= read -r line; do
      ...
      if [ "$status" = "D" ]; then
        if remove_file "$TARGET_RULES_DIR/$(basename "$file")" "$(basename "$file")"; then
          ((deleted++)) || true
        fi
      else
        if copy_file "$SCRIPT_DIR/$file" "$TARGET_RULES_DIR/$(basename "$file")" "false"; then
          ((added++)) || true
        fi
      fi
    done <<< "$changed"
...
echo -e "${GREEN}========================================${NC}"
echo ""
echo "Local .claude directory: $TARGET_CLAUDE_DIR"
echo ""
